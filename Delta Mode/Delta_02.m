...
% Reset
    ...
    fprintf(obj2,':SENSe:VOLTage:DELTa OFF')     % Enable or disable Delta
    fprintf(obj2,':SYSTem:faz off')     % Ignore initial DC offset
    ...
    fprintf(obj2,':TRACe:POINts 20')          % Specify size of buffer; 2 (because delta takes at least 2 values) to 1024
    %fprintf(obj2,':TRACe:FEED:CONTrol NEver')   % Select buffer control mode (NEXT or NEVer)
    fprintf(obj2,':trac:feed:cont next')        % Enable buffer.
...
%     fprintf(obj1,':TRIGger:DIRection SOURce')   % Enable (SOURce) or disable (ACCeptor) bypass           
    ...
    fprintf(obj1,':SENSe:VOLTage:PROTection:LEVel 1')     % Specify voltage limit for I-Source 
    ...
    fprintf(obj1,':SENSe:VOLTage:NPLCycles 0.01')           % Specify integration rate (in line cycles):
     fprintf(obj1,':route:terminals rear');
    fprintf(obj1,':SOURce:CURRent:MODE LIST')   % Select I-Source mode (FIXed, SWEep, or LIST).
    fprintf(obj1,':SOURce:LIST:CURRent 0.1E-6,-0.1E-6')       % Create list of I-Source values
 ...
 data=query(obj2, ':trace:free?')
   vals = str2double(strsplit(data,','));
        
        while vals(2)./18<count-1 
     data=query(obj2, ':trace:free?')
   vals = str2double(strsplit(data,','));
        ...
        fprintf(obj2,':trace:feed:cont nev');
        data = query(obj2, ':trace:data?');         % Request all stored readings
vals = str2double(strsplit(data,','));      % Export readings to array

		data=query(obj1,':TRAC:DATA?')
		I = str2double(data,',','%f');
