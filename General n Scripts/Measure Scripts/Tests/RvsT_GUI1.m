...
% First Measurement
TEMP(PPMS_obj,10,5,10,1);    % Set Temperature to 10[°K] in rate 5[°K/min]
...
while Status(1)~=1 && Status(1)~=4    % while Temperature not stable
    ...
    for s = 1:length(close_ind)
        ...
        [Id,Vd] = deltaExecuteSpan(nv_obj, sm_obj,'C',0,0.1,1);    % Delta measurement (Current): center 0[uA] , span  0.1[uA] , 1 repeats
...
% Second measurement
TEMP(PPMS_obj,2,0.1,10,1);    % Set Temperature to 2[°K] in rate 0.1[°K/min]
[Status, StatusTXT] = ReadPPMSstatus(PPMS_obj);    % Read status
while Status(1)~=1 && Status(1)~=4    % while Temperature not stable
    % Scan on switcher configurations
    for s = 1:length(close_ind)
...
FileName1 = 'C:\Users\ppms\Documents\MATLAB\Rack-O\General n Scripts\Measure Scripts\Tests\RvsTRvsT_GUI';    % original File Name
